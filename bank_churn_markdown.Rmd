...
#This plot is too crowded to use easily. We will include iot in our findings to show the full picture of the data, but for now we can use two separate decision trees with Age being the main factor in both: dtree1 using Age and NumOfProducts and dtree2 using Age and Balance. This will give us clearer visuals that have a similar accuracy to the first tree.
...
#The decision trees with two factors each are easier to read and have comparable accuracy to the original tree, so we will include them in our findings
```